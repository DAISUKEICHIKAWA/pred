...
#head(tdc)
#層別解析(性別の指定)
sex_flag <- c("male")##### 性別の指定(男性，女性以外を入力すると，全データ)
if(sex_flag == "male"){
...
####学習データから，リスク，非リスクが同じ割合になるように復元抽出を認め，学習データを加工
s = strata(Train,stratanames=c(out),size=c(nrow(Train)/2,nrow(Train)/2),method="srswr")
Train <- getdata(Train,s)
#### validation data をリスクと同じ数の非リスクを抽出
s = strata(Valid,stratanames=c(out),size=c(table(Valid[c(out)])[2],table(Valid[c(out)])[2]),method="srswor")
Valid <- getdata(Valid,s)
...
logi.f.Sens <- logi.f.ROC$res[logi.f.mhv[1],1]#Sensitivity
logi.f.Spec <- logi.f.ROC$res[logi.f.mhv[1],2]#Specificity
logi.f.Score <- logi.f.ROC$res[logi.f.mhv[1],5]#Score
...
logi.s.Sens <- logi.s.ROC$res[logi.s.mhv[1],1]#Sensitivity
logi.s.Spec <- logi.s.ROC$res[logi.s.mhv[1],2]#Specificity
logi.s.Score <- logi.s.ROC$res[logi.s.mhv[1],5]#Score
...
LASSO.Sens <- LASSO.ROC$res[LASSO.mhv[1],1]#Sensitivity
LASSO.Spec <- LASSO.ROC$res[LASSO.mhv[1],2]#Specificity
LASSO.Score <- LASSO.ROC$res[LASSO.mhv[1],5]#Score
...
#### 100%リスクのvalidation data　作成
Validp <- subset(Valid,Valid$Stratum==1)

logit_f.valid <- predict(logit.full,newdata = Validp)
logit_f.ROC <- data.frame(cbind(Valid[,c(out)],logit_f.valid))
colnames(logit_f.ROC) <- c("Y","score")
logit_f.ROC$flag <- as.numeric(cut(logit_f.ROC$score, right=FALSE, breaks=c(-Inf,logi.f.Score, Inf), labels=c(0,1)))-1

logit_s.valid <- predict(logit.step,newdata = Validp)
logit_s.ROC <- data.frame(cbind(Valid[,c(out)],logit_s.valid))
colnames(logit_s.ROC) <- c("Y","score")
logit_s.ROC$flag <- as.numeric(cut(logit_s.ROC$score, right=FALSE, breaks=c(-Inf,logi.s.Score, Inf), labels=c(0,1)))-1

LASSO.valid <- predict(glmnet,newx=do.call(cbind,Valid[c(pred)]),family="binomial")
LASSO.ROC <- data.frame(cbind(Valid[,c(out)],LASSO.valid))
colnames(LASSO.ROC) <- c("Y","score")
LASSO.ROC$flag <- as.numeric(cut(LASSO.ROC$score, right=FALSE, breaks=c(-Inf,LASSO.Score, Inf), labels=c(0,1)))-1

#陽性的中率
PVp.full <- table(logit_f.ROC$flag)[2]/(table(logit_f.ROC$flag)[1]+table(logit_f.ROC$flag)[2])
PVp.stepwise <- table(logit_s.ROC$flag)[2]/(table(logit_s.ROC$flag)[1]+table(logit_s.ROC$flag)[2])
PVp.LASSO <- table(LASSO.ROC$flag)[2]/(table(LASSO.ROC$flag)[1]+table(LASSO.ROC$flag)[2])
PVp <- rbind(PVp.full,PVp.stepwise,PVp.LASSO)
rownames(PVp) <- c("full.model","stepwise.model","LASSO.model")
colnames(PVp) <- c("PV+")
res4 <- cbind(res3,PVp)
